...
git config --global user.name "SamuelKuronuma"
git config --global user.email "samuel.kuronuma@etec.sp.gov.br"

:: Adiciona o repositório remoto (só executa se já não estiver adicionado)
git remote get-url origin >nul 2>&1
if errorlevel 1 (
    git remote add origin "https://github.com/SamuelKuronuma/RepositoriJavaBasico.git"
)
